...
```{r}
data.latest.all <- data_total %>% filter(date == max(date)) %>% data_total[,!(names(data_total) %in% c("new.confirmed","new.deaths","new.recovered","rate.upper","rate.lower","rate.daily"))] %>% mutate(ranking = dense)
