...
head(dataDate)
data.latest.all <- dataDate[,c("country","date","confirmed","new.confirmed","recovered","deaths","new.deaths","death.rate")]
...
top.countries <- data.latest.all[data.latest.all$ranking <= k+1,] %>% arrange(ranking)%>% setdiff('World')
