...
datafromserver <- function(){
  serverdb3 <- src_mysql(dbname = "covid19", host = "cvd-database.cmxpnyylhkiw.us-east-2.rds.amazonaws.com", user = "admin", password = "RprojectCovid19")
...
datafromserver.big <-function(){
  serverdb4 <- src_mysql(dbname = "covid19", host = "cvd-database.cmxpnyylhkiw.us-east-2.rds.amazonaws.com", user = "admin", password = "RprojectCovid19")
...
```{r}
## profiling function of connecting data
profvis({
  datafrommysql()  
  datafromserver()
  datafrommysql.big()
...
# 2.1 function cleandata with different size of data in "Local database"
...
```{r}
# 2.2 function cleandata with different size of data in "Server"
...
# Local 
profvis({
  small_merge <- MergeData(small_confirmed, small_deaths, small_recover)
  big_merge <- MergeData(big_confirmed, big_deaths, big_recover)

})

# Server
profvis({
  small_merge.server <- MergeData(small_confirmed.server, small_deaths.server, small_recover.server)
  big_merge.server <- MergeData(big_confirmed.server, big_deaths.server, big_recover.server)
})
```
...
profvis({
  # Local
  small_dataFormat <- small_merge %>% ChangeDataFormat()
  big_dataFormat <- big_merge %>% ChangeDataFormat()
  
  # Server
  small_dataFormat.server <- small_merge.server %>% ChangeDataFormat()
  big_dataFormat.server <- big_merge.server %>% ChangeDataFormat()
})
```
...
profvis({
  small_top20 <- small_daily %>% CalcTop20()
  big_top20 <- big_daily %>% CalcTop20()
  
  small_top20.server <- small_daily.server %>% CalcTop20()
  big_top20.server <- big_daily.server %>% CalcTop20()
})
```
