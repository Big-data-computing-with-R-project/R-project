...
profvis({
  ...
  df_conf %<>% select(-c(Province.State, Lat, Long)) %>% rename(country = Country.Region)
  ## convert from wide to long format
...
nd_long <- function(data){
  data.long <- data[,!(names(data) %in% c("new.confirmed","new.deaths","new.recovered","rate.upper","rate.lower","rate.daily"))]
  ...
  rates.long <- data[,!(names(data) %in% c("new.confirmed","new.deaths","new.recovered","confirmed","current.confirmed","recovered","deaths"))]
