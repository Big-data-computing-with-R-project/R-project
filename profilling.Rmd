...
nd_top20 <- function(data){
  dataDate <- data[data$date  == "2020-08-31",]%>%rename(c("rate.lower" = "death.rate"))
...
profvis({
  # d_top 20: Top 20 countries with no dplyr
  data1 <- data %>% nd_top20() 
...
profvis({
  # d_top 20: Top 20 countries with dplyr
  #data2 <- data %>% d_top20()
  data.latest.all <- data %>% filter(date == max(date)) %>%
  select(country, date,confirmed, new.confirmed, current.confirmed,
         recovered, deaths, new.deaths, death.rate=rate.lower) %>%
...
profvis({
  ...
  #not use pipe
  data.latest.all <- filter(data, date == max(date)) 
  data.latest.all <- select(data.latest.all, country, date,confirmed, new.confirmed, current.confirmed,
         recovered, deaths, new.deaths, death.rate=rate.lower) 
  data.latest.all <- mutate(data.latest.all, ranking = dense_rank(desc(confirmed))) 
